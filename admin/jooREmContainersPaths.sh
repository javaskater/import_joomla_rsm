...
res=$(mysql -u${JOO_MYSQL_ID} -p${JOO_MYSQL_ID} ${JOO_MYSQL_ID} -s -N -e "SELECT id,filepath FROM jo25_downloads_containers where filepath <> ''")
#echo "resultat:"$res
...
do
	...
	then
		...
		sql_command="update jo25_downloads_containers set filepath='${path_joo_local}' where id=${id_joo}"
