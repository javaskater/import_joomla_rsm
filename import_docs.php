	...
	class JooRsm_docs {
		...
		public function suppress_all_ahm_data($action){
			...
			if($action == 'all'){
				...
				$sql_delete_ahm = sprintf("DELETE FROM `%s`","ahm_files");
				$del_sql = $wpdb->query($sql_delete_ahm);
				if (!$del_sql){
					if($wpdb->last_error){
						$this->caller->display_admin_error(sprintf('la requete de nettoyage de la table des fichiers joints:%s a eu un probleme %s', $sql_delete_ahm,$wpdb->last_error));
					}else{
						$this->caller->display_admin_notice(sprintf('la requete de nettoyage de la table des fichiers joints:%s a rien fait, la table etait deja vide',$sql_delete_ahm));
					}
				}
				//Delete the Option 
				if(!delete_option("ahm_files")){
					$this->caller->display_admin_notice("erreur a la supression de l'option ahm_files qui contient les catégories peut etre elle n existe pas?");
